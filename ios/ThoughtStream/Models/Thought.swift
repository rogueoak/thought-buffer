...
/// A time range inside a thought's recording, in seconds from the start of the recording. Attached
/// to a committed paragraph so playback can seek straight to it (see `AudioThoughtPlayer`).
...
/// A single captured thought: a title plus an ordered list of paragraphs.
///
/// Thoughts persist as Markdown files (see `ThoughtStore`). The value type stays small and
...
///
/// A thought MAY carry a recording of the voice that produced it (spec 0007): `audioFileName` names
/// the sibling `.m4a` next to the thought's `.md`, and `timings` maps each paragraph to its range in
/// that recording. Both are optional and tolerant - a thought with no audio (older files, or capture
/// with recording turned off) loads and behaves exactly as a text-only thought always has.
struct Thought: Identifiable, Hashable {
    ...
    /// (spec 0009). It governs edit-time behavior: a non-custom thought re-derives its title when the body
    /// changes; a custom thought keeps the user's title. Persisted as `titleCustom: true` in frontmatter,
    /// written ONLY when true so a derived-title thought serializes byte-for-byte as before.
    let hasCustomTitle: Bool
    /// The name of the sibling audio file (`<id>.m4a`), or nil when the thought has no recording.
    let audioFileName: String?
    /// One timing per paragraph, in order, or empty when the thought has no recording. When present
    ...
    /// The ordered folder names containing this thought, empty for a top-level thought (spec 0010). This is
    /// a STORAGE LOCATION, not thought content: it is derived on load from the relative directory the
    /// file sits in, and consumed on save to place the file under `directory/<folderPath>/<id>.md`.
    /// It is NEVER serialized into the Markdown - a thought's folder is where its file lives, not a
    /// frontmatter key - so a foldered thought and a top-level thought produce byte-identical `.md` output
    ...
    /// The number of paragraphs in the thought.
    ...
    /// Shown on the thought card (feedback 0005) instead of a paragraph count, which read oddly ("1
    ...
    /// A short, correctly-pluralized word-count label ("1 word", "12 words") for the thought card.
    ...
    var recordingDurationLabel: String {
        Thought.durationLabel(recordingDuration)
    }

    /// The at-a-glance stat shown beside the timestamp on the card and detail header (feedback 0010):
    /// the recording duration for a thought that has audio, falling back to the word count for a
    /// text-only thought (transcript-only retention, resumed/edited thoughts, older files) so it is never
    ...
    /// Whether this thought carries a recording. Both an audio filename AND at least one timing are
    ...
    ///   key with no `timings:` (or vice versa) is treated as a text-only thought, so a half-written or
    ...
    /// `start + duration` across timings). Zero when the thought has no timings. Used to show the
...
extension Thought {
    ...
    /// Derive a human title from the thought's paragraphs, or a dated fallback when empty.
    ...
    static func deriveTitle(paragraphs: [String], createdAt: Date) -> String {
        ...
        return "Thought " + Thought.fallbackDateFormatter.string(from: createdAt)
    }

    /// Whether a brand-new thought's editor buffer is still a blank draft (spec 0013): no non-whitespace
    ...
    /// A copy of this thought with the edited fields applied, PRESERVING its storage location
    ...
    /// `currentThought`. A thought that lives in a folder MUST keep its `folderPath` here, or `ThoughtStore.save`
    /// (which files by `folderPath`) would relocate it to the root on every edit.
    func editedCopy(paragraphs: [String], hasCustomTitle: Bool, customTitle: String) -> Thought {
...
extension Thought {
    /// Sanitize a user-entered folder name so it cannot escape the thought tree. This REJECTS unsafe
    ...
    /// A copy of this thought relocated to `folderPath`. The store uses it on load to tag a thought with
    /// the folder its file was found in; keeping this a pure copy keeps `folderPath` a `let`.
    func withFolderPath(_ folderPath: [String]) -> Thought {
        Thought(
            id: id,
            title: title,
            paragraphs: paragraphs,
...
extension Thought {
    /// The thought as plain text for sharing / copying (spec 0017): the title on its own line, a blank
    ...
    /// `bodyMarkdown`'s joining), but the output is plain text - no Markdown, no frontmatter. A thought
    ...
    ///
    /// This leans on `bodyMarkdown`'s CURRENT contract that a paragraph is plain prose (no inline
    /// Markdown), so the joined body is already the plain text we want to share. If inline Markdown is
    /// ever introduced into thought bodies, sharing/export should move to a dedicated `ThoughtExport`
    /// formatter that strips or renders the markup, rather than re-using `bodyMarkdown` here.
...
extension Thought {
    ...
    ///
    /// The `audio` and `timings` keys are written only when the thought has a recording, so a
    /// text-only thought serializes byte-for-byte as it always has. `timings` is a compact JSON array
    /// of `[start, duration]` pairs (seconds), one per paragraph; an old parser ignores both keys.
    var markdown: String {
        let created = Thought.iso8601.string(from: createdAt)
        var lines = [
            "---",
            "id: \(id.uuidString)",
            "title: \(Thought.escapeYAML(title))",
            "created: \(created)",
        ]
        // Only mark a USER-set title, so a derived-title thought serializes exactly as before (spec 0009).
        if hasCustomTitle {
            lines.append("titleCustom: true")
        }
        if let audioFileName {
            lines.append("audio: \(Thought.escapeYAML(audioFileName))")
        }
        if !timings.isEmpty {
            lines.append("timings: \(Thought.encodeTimings(timings))")
        }
        lines.append("---")
    ...
    /// value yields an empty array (the thought simply behaves as text-only) rather than failing the
    /// whole parse.
    ...
    /// Parse a thought from Markdown file contents. Tolerant: a file without frontmatter still
    /// loads as a body-only thought. `fallbackDate` is used when no `created` key is present
    ...
    init(markdown text: String, fallbackID: UUID = UUID(), fallbackDate: Date = Date()) {
        ...
        if let front = Thought.extractFrontmatter(text) {
            body = front.body
            for (key, value) in front.fields {
                ...
                case "title":
                    let unescaped = Thought.unescapeYAML(value)
                    if !unescaped.isEmpty { title = unescaped }
                case "titleCustom":
                    hasCustomTitle = (value == "true")
                case "created":
                    if let parsed = Thought.iso8601.date(from: value) { createdAt = parsed }
                case "audio":
                    let unescaped = Thought.unescapeYAML(value)
                    if !unescaped.isEmpty { audioFileName = unescaped }
                case "timings":
                    timings = Thought.decodeTimings(value)
                default:
        ...
        self.paragraphs = paragraphs
        self.createdAt = createdAt
        // A parsed thought carries no folder: the folder is the file's LOCATION, tagged by the store on
        ...
        // real recording, so treat it as a text-only thought (backward compatible).
        ...
        if let title {
            ...
            self.title = Thought.deriveTitle(paragraphs: paragraphs, createdAt: createdAt)
