...
/// Watches the iCloud notes folder for changes - external edits, or files synced in from another
...
///
/// The production implementation is `MetadataUbiquitousNoteObserver`, backed by `NSMetadataQuery`
...
protocol UbiquitousNoteObserving: AnyObject {
    /// Called on the main actor whenever the set of iCloud note files changes. The observer
    /// coalesces query updates; the receiver typically reloads via its `NoteStoring`.
...
/// Pure mapping from iCloud metadata items to the note file URLs the store should load, plus the
...
enum UbiquitousNoteMapping {
    ...
    /// The `.md` note files among the items, sorted by last path component for a stable order.
    static func noteURLs(from items: [Item]) -> [URL] {
        items
            .map(\.url)
    ...
    /// The `.md` note files that are not yet downloaded and so need a download kicked off.
...
/// Live observer backed by `NSMetadataQuery`. Enumerates iCloud note documents, triggers
...
final class MetadataUbiquitousNoteObserver: NSObject, UbiquitousNoteObserving {
    var onChange: (() -> Void)?
    ...
    override init() {
        ...
        // Only markdown note files under the container Documents area.
    ...
    @objc private func queryDidUpdate(_ notification: Notification) {
        ...
        var items: [UbiquitousNoteMapping.Item] = []
        ...
        // Kick downloads for anything present in metadata but not yet local, so external notes
        // become readable.
        for url in UbiquitousNoteMapping.urlsNeedingDownload(from: items) {
