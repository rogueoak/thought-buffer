...
///
/// A note's title is its first dictated line, which may be sensitive. Since playback lights up the
...
enum LockScreenTitle: String, CaseIterable, Identifiable {
    /// Show the note's own title (its first line). The default.
    case noteTitle
    /// Show a fixed, generic label instead of the note title, so a sensitive first line never
    /// appears on the lock screen / Control Center / CarPlay.
    case generic

    var id: String { rawValue }

    /// The default: show the note title.
    static let `default`: LockScreenTitle = .noteTitle

    /// The fixed label published when `generic` is selected.
    static let genericTitle = "Thought Stream recording"

    /// The Now Playing title to publish for `note` under this setting: the note's own title, or the
    /// fixed generic label when the user has chosen to hide it.
    func nowPlayingTitle(for noteTitle: String) -> String {
        switch self {
