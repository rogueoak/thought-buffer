    ...
    var body: some View {
        VStack(alignment: .leading, spacing: CanopySpacing.x2) {
            ...
            HStack(spacing: CanopySpacing.x3) {
                // The clock/time-since-created and timer/duration pairs are shared with the thought detail
                // header via `ThoughtMetaStats` (feedback 0013 tightens both to `x1`; feedback 0015 gives
                // the detail duration the same timer glyph) so the card and the detail page can't drift.
                ThoughtMetaStats(thought: thought)
                // A small play affordance so a thought with a recording is discoverable at a glance
