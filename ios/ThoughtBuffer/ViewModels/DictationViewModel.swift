...
final class DictationViewModel: ObservableObject {
    ...
    enum DeniedReason: Equatable {
        ...
        var detail: String {
            switch self {
            case .speechNotAuthorized:
                return "Thought Buffer needs speech recognition to turn your voice into thoughts. "
                    + "Turn it on in Settings, under Thought Buffer."
            case .microphoneNotAuthorized:
                return "Thought Buffer needs the microphone to hear you. Turn it on in Settings, "
                    + "under Thought Buffer."
            case .onDeviceUnavailable:
    ...
    private func takeResumeSegmentForConcatenation(savedThought: Thought) -> URL? {
        ...
        let owned = fm.temporaryDirectory
            .appendingPathComponent("thoughtbuffer-resume-\(UUID().uuidString).\(ThoughtStore.audioFileExtension)")
        do {
