...
///
/// The root is a `CPListTemplate` with two sections - a "Start a thought" row that calls the
...
final class CarPlaySceneDelegate: UIResponder, CPTemplateApplicationSceneDelegate {
    ...
    func templateApplicationScene(
        ...
        let root = CPListTemplate(title: "Thought Buffer", sections: [])
    ...
    /// Build the "Start a thought" row, whose tap requests a new session through `onStart`.
    ...
    static func makeStartItem(onStart: @escaping () -> Void) -> CPListItem {
        let start = CPListItem(text: "Start a thought", detailText: "Begin a new thought by voice")
