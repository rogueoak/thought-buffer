...
/// "Hey Siri, start a thought in Thought Buffer." Launches the app and begins a fresh dictation
...
struct StartThoughtBufferIntent: AppIntent {
    static var title: LocalizedStringResource = "Start a Thought"
    static var description = IntentDescription(
        "Open Thought Buffer and start a new dictation session, hands-free."
    )
...
/// "New thought in Thought Buffer." A second, cheap entry that starts the same fresh session. In this
/// app starting a thought and adding a new thought both mean "open a fresh dictation session", so it
/// shares the one starter; the separate phrase just gives Siri and Shortcuts a second natural way in.
struct NewThoughtIntent: AppIntent {
    static var title: LocalizedStringResource = "New Thought"
    static var description = IntentDescription(
        "Open Thought Buffer and start a fresh thought by voice."
    )
...
/// \(.applicationName)"`.
struct ThoughtBufferShortcuts: AppShortcutsProvider {
    /// The leading text of each "start a thought" phrase; the app name follows in the literal below.
    /// Deliberately share NO exact phrase with `newThoughtPhraseLeads`, so Siri never has to
    /// disambiguate an identical phrase between the two intents.
    ...
    static var appShortcuts: [AppShortcut] {
        ...
        AppShortcut(
            ...
            phrases: [
                ...
                "Add a thought in \(.applicationName)"
            ],
