...
/// The Share + Copy actions for a thought (spec 0017), factored into ONE place so the thought detail
...
///
/// Share sends `thought.shareableText` through the system share sheet (`ShareLink`); Copy text puts the
/// same string on the pasteboard through `ThoughtClipboard.copy` and calls `onCopied` so the caller can
/// flash its confirmation. Keeping both on `thought.shareableText` is what makes share and copy identical.
struct ThoughtActionsMenu<Extra: View>: View {
    let thought: Thought
    /// Called after Copy text places the thought on the pasteboard, so the caller flashes its transient
    ...
    init(thought: Thought, onCopied: @escaping () -> Void, @ViewBuilder extraContent: () -> Extra) {
    ...
    var body: some View {
        ShareLink(item: thought.shareableText) {
            Label("Share", systemImage: "square.and.arrow.up")
        }
        Button {
            ThoughtClipboard.copy(thought)
            onCopied()
        } label: {
...
extension ThoughtActionsMenu where Extra == EmptyView {
    /// Share + Copy with no extra items.
    init(thought: Thought, onCopied: @escaping () -> Void) {
        self.init(thought: thought, onCopied: onCopied, extraContent: { EmptyView() })
    }
}

/// The one place a thought's shareable text is written to the pasteboard, shared by every Copy-text site
/// (spec 0017) so they all copy exactly `thought.shareableText`.
enum ThoughtClipboard {
    static func copy(_ thought: Thought) {
        UIPasteboard.general.string = thought.shareableText
    }
}

/// The transient "Copied to clipboard" confirmation chip (spec 0017), shared by the thought detail page
...
/// The transient "Thought deleted - Undo" affordance (spec 0020): a muted capsule matching the copied chip,
...
struct UndoDeleteAffordance: View {
    /// Called when the user taps Undo. Restores the thought (see `ThoughtDeletionController.undo`).
    let onUndo: () -> Void
