...
/// `Documents/ThoughtBuffer/`, so thoughts sync across a user's devices and appear in the Files app.
///
...
struct ICloudThoughtStore: ThoughtStoring {
    /// The `ThoughtBuffer/` directory inside the container's `Documents`. This is where the thoughts
    ...
    /// Build a store rooted at `<containerDocuments>/ThoughtBuffer/`.
    ///
    ...
    init(containerDocumentsURL: URL) {
        self.directory = containerDocumentsURL.appendingPathComponent("ThoughtBuffer", isDirectory: true)
    }
    ...
    /// is the exact thoughts dir, that one is the container Documents that gets `ThoughtBuffer/`
