...
Dead-air removal REPLACES a recording with no kept original, so a botched trim is unrecoverable data loss. The safe shape is never "overwrite the file": write the trimmed audio to a TEMP file, VERIFY it is a real non-empty audio file (it opens via `AVAudioFile` and has frames), and only THEN `replaceItemAt` the original atomically - and swallow EVERY failure (unreadable source, nothing to trim, a write/verify slip, an empty result) into a "not trimmed" fallback that leaves the original untouched and lets the note save anyway. The recording is never sacrificed for the sake of the trim. Three more rules fell out of building it. First: carve the risky logic into PURE cores tested without audio - `SilenceTrimmer` (windowed RMS -> keep-ranges, with named `silenceFloor`/`minPauseSeconds`/`breathGapSeconds` constants and every edge: silence at start/middle/end, back-to-back, none, all-silent, sub-threshold) and `TimingRemapper` (shift paragraph starts by removed duration, durations unchanged) - so the AVFoundation glue stays thin and the parts that actually corrupt data are provable in CI. Second: `AVAssetExportSession` + `AVURLAsset.tracks(...)` is a trap on modern iOS - the synchronous track accessor is load-gated and the export returned a bare `nilError`, so the composition-and-export path silently failed to `notTrimmed`; concatenating the kept frame ranges by straight `AVAudioFile` read/write is deterministic, self-contained, and has no async load/export to lose. Third: a merge/remap that is only correct because two thresholds sit on a particular side of each other (here trim floor 2.0s > paragraph-gap 1.5s, so a trimmable silence is always a paragraph boundary and never inside a paragraph) needs a GUARD TEST asserting the inequality, so a later tweak to either constant is forced to reconsider the remap rather than silently break the paragraph<->time mapping. Generalizes to any destructive/lossy transform of a user artifact: keep the numeric decision pure and exhaustively tested, do the mutation as verify-then-atomic-replace with a total failure fallback, prefer a direct read/write over a load-gated framework pipeline when the transform is simple, and pin any cross-constant invariant the transform depends on with its own assertion.
